...
set_environment() {
    if ! grep -q "\$HOME/.config/composer/vendor/bin" "$PROFILE"; then
        echo -e "\n## PHP" >>"$PROFILE"
...
install() {
    ...
    if [ ! -f "$COMPOSER_PATH" ]; then
        sudo curl -fSL -o "$COMPOSER_PATH" -fsL "$COMPOSER_DOWNLOAD_URL"
